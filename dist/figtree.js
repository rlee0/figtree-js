...
import { isArray, isValidChild, onAction, onParam, replaceProps, walk } from "./util";
...
function figtree(config) {
  return (set, get) => {
    ...
    const applyElements = obj => {
      ...
      const replacedElement = R.assocPath(convertPath("props.data-path"), `elements.${type}`)(R.mergeDeepRight(elements[type], rest));
    ...
    const renderTree = (obj, key) => {
      ...
      const {
        type,
        ...rest
      } = applyActions(applyElements(obj, key), key);
      ...
      return /*#__PURE__*/React.createElement(Component, props, renderTree(rest.children, `${key}.children`));
    };

...
const Figtree = ({ store, entry }) => {
  const { api, elements } = store();
...
const figtree = (config) => (set, get) => {
  ...
  const applyElements = (obj) => {
    ...
    const element = elements ? elements[type] : { type };
  ...
  const applyProps = (obj) => {
    ...
    const nextObj = JSON.parse(
      JSON.stringify(obj).replace(/"[^"]*({{.*?}})[^"]*"/g, (match, value) => {
  ...
  const generateTree = (obj, path) => {
    const generateLeaf = (o, p) => {
      ...
      const { type, props, children } = R.pipe(applyElements, applyActions)(o);
      return {
        type,
        props: { ...props, key },
  ...
  const renderTree = (obj) => {
    ...
    const props =
      Component === React.Fragment
        ? { key: obj.props.key }
        : { ...obj.props, ["data-path"]: obj.props.key };
    return <Component {...props}>{renderTree(obj.children)}</Component>;
  };
